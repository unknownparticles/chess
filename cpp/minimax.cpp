...
std::vector<GameMove> MinimaxAI::filterSafeRootMoves(
    GameEngine &engine,
    const std::vector<GameMove> &moves) const
{
    ...
    GomokuEngine *gomoku = dynamic_cast<GomokuEngine *>(&engine);
    ...
    for (GameMove move : moves)
    {
        ...
        bool dangerous =
            gomoku->allowsOpponentDoubleThreat(move, aiPlayer);
